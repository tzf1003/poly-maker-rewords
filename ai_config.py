...
**⚠️ 重要约束**：
- `trade_size` 必须 < `max_size`（通常 trade_size = max_size / 4）
...
- `mid`: 中等风险（止损 -1.50%, 止盈 +1.50%, 价差阈值 0.04, 休眠期 2h）- **推荐**
...
6. **工具使用规范** ⚠️ 重要：
   - 调用 `update_selected_markets` 工具时，**必须提供 markets 参数**
...
**重要规则**：
1. **必须始终提供选择列表**（即使不改变当前列表）
2. 优先保留当前表现良好的市场
3. 只有在发现明显更好的市场时才替换
4. 确保市场数量不超过 {max_markets} 个
5. **只有在所有市场表现均不好时**，才可以提交空列表（极少情况）
...
请使用 `update_selected_markets` 工具更新选择，并解释你的决策。
"""
