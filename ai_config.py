...
* **max_size 不能超过实际钱包余额**（因挂单验证）
* 钱包余额**不需要平均分配**到多个市场
* 每个市场可以**独立设置** max_size
* 示例：钱包余额 200 USDC，3 个市场各设 max_size=100
...
* trade_size 通常设置为 max_size 的约 1/4，用于渐进式建仓
...
* `volatilty/reward`: 波动率/奖励比（越低越好）= gm_reward_per_100 / volatility_sum
...
* `trade_size` 必须 ≥ `min_size`
* `best_bid` 和 `best_ask` 必须在 [0.10, 0.90] 区间内（避免极端价格）
* 示例：max_size = 500, trade_size = 125（分 4 次建满仓）
...
* `mid`: 中风险（止损 -1.50%, 止盈 +1.50%, 价差阈值 0.04, 休眠期 2h）
...
1. 价格范围：`best_bid >= 0.1` 且 `best_ask <= 0.9`
2. 流动性：`spread < 0.15`
3. 规模：`min_size <= trade_size`
